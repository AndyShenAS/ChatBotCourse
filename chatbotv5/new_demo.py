...
wordVector = word_vector.WordVector()
...
output_seq_len = 5
# 空值填充0
PAD_ID = np.zeros(wordVector.word_vec_dim)
# 输出序列起始标记
# GO_ID = np.ones(wordVector.word_vec_dim)
# 结尾标记
# EOS_ID = 2*np.ones(wordVector.word_vec_dim)
# LSTM神经元size
size = 8
# 初始学习率
...
max_token_id = len(wordVector.word_vector_dict)
num_encoder_symbols = max_token_id + 5
num_decoder_symbols = max_token_id + 5


def get_vec_list_from(sentence_list):
    sentence_vec_list = []
    for str in sentence_list:
        vec = wordVector.word2vector(str)
        if vec:
        ...
        with open('./samples/backup/answer', 'r') as answer_file:
            while True:
                ...
                if question and answer:
                    ...
                    answer_list = jieba.cut(answer)
                    answer_list.insert(0,'GO_ID')
                    answer_list.append('EOS_ID')

                    question_vec_list = get_vec_list_from(question_list)
                    answer_vec_list = get_vec_list_from(answer_list)
                    if len(question_vec_list) > 0 and len(answer_vec_list) > 2:
                        train_set.append([question_vec_list, answer_vec_list])
                else:
                    break
    return train_set
...
def get_samples(train_set, batch_num):
    ...
    :return:
        ...
        decoder_inputs: [array([1, 1], dtype=int32), array([11, 11], dtype=int32), array([13, 13], dtype=int32),
                        array([15, 15], dtype=int32), array([2, 2], dtype=int32)]  dtype=tf.float32
    """
    # train_set = [[[5, 7, 9], [11, 13, 15, EOS_ID]], [[7, 9, 11], [13, 15, 17, EOS_ID]], [[15, 17, 19], [21, 23, 25, EOS_ID]]]
    raw_encoder_input = []
    raw_decoder_input = []
    raw_mark_decoder_input = []
    ...
    for sample in batch_train_set:
        ...
        raw_decoder_input.append(sample[1] + PAD_ID * (output_seq_len - len(sample[1])))
        raw_mark_decoder_input.append([1] * len(sample[1] + [0] * (output_seq_len - len(sample[1])))

    encoder_inputs = []
    decoder_inputs = []
    target_weights = []

    #按照input_seq_len的长度，把所有样本中同一位置的词加到一个array里
    for length_idx in range(input_seq_len):
        encoder_inputs.append(np.array([encoder_input[length_idx] for encoder_input in raw_encoder_input], dtype=np.float32))
    for length_idx in range(output_seq_len):
        decoder_inputs.append(np.array([decoder_input[length_idx] for decoder_input in raw_decoder_input], dtype=np.float32))
        target_weights.append(np.array([
            np.zeros(wordVector.word_vec_dim) if decoder_mark_input[length_idx] == 0 else np.ones(wordVector.word_vec_dim) for decoder_mark_input in raw_mark_decoder_input
        ], dtype=np.float32))
    return encoder_inputs, decoder_inputs, target_weights


def seq_to_encoder(input_vec_list):
    """从输入对话vec列表，转成预测用的encoder、decoder、target_weight等
    """
    encoder_input = PAD_ID * (input_seq_len - len(input_vec_list)) + input_vec_list
    decoder_input = wordVector.word_vector_dict['GO_ID'] + PAD_ID * (output_seq_len - 1)
    encoder_inputs = [np.array([v], dtype=np.float32) for v in encoder_input]
    decoder_inputs = [np.array([v], dtype=np.float32) for v in decoder_input]
    target_weights = [np.array([np.ones(wordVector.word_vec_dim)], dtype=np.float32)] * output_seq_len
    return encoder_inputs, decoder_inputs, target_weights
...
def predict():
    ...
    with tf.Session() as sess:
        ...
        while input_seq:
            ...
            input_vec_list = get_vec_list_from(input_seq)
            if (len(input_vec_list)):
                sample_encoder_inputs, sample_decoder_inputs, sample_target_weights = seq_to_encoder(input_vec_list)
