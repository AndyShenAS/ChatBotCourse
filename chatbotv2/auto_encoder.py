...
def load_word_set():
    file_object = open('./corpus.segment', 'r')
    while True:
...
def init_seq(input_file):
    ...
    while True:
        ...
        if line:
            ...
            # print("line_question:",line_question)
            # print("line_answer:",line_answer)
...
class MySeq2Seq(object):
    ...
    """
    def __init__(self, max_seq_len = 16, word_vec_dim = 200, input_file='./corpus.segment'):
    ...
    def generate_trainig_data(self):
        ...
        # print("question_seqs[0:5]...",question_seqs[0:5])
        # print("question_seqs[0:5]...",answer_seqs[0:5])
        xy_data = []
        y_data = []
        count = 0
    ...
    def model(self, feed_previous=False):
        ...
        (encoder_output_tensor, states) = tflearn.lstm(encoder_inputs, self.word_vec_dim, return_state=True, scope='encoder_lstm', name="encoder_lstm")
        hidden_layer_output = encoder_output_tensor
        hidden_layer_states = states
...
# Only used when state_is_tuple=True.
        print("type(hidden_layer_output): ",type(hidden_layer_output))
        print("type(hidden_layer_states): ",type(hidden_layer_states[0]))
        #下面这句没用
        encoder_output_sequence = tf.stack([encoder_output_tensor], axis=1, name = "enc_out" )
        # if feed_previous:
        #     with tf.Session() as sess:
        #         print(sess.run(encoder_output_tensor))
        ...
        # net = tflearn.regression(real_output_sequence, optimizer='adam', learning_rate=0.000001, loss='my_mean_square')
        ...
        return model,hidden_layer_output,hidden_layer_states
    ...
    def train(self):
        ...
        # print(trainXY[:1])
        # print(trainY[:1])
        model,_,_ = self.model(feed_previous=False)
        # model.load('./model/autoencoder_model')
        model.fit(trainXY, trainY, n_epoch=100000, snapshot_epoch=False, batch_size=2000)
        model.save('./model/autoencoder_model')
        return model

    def load(self):
        model,hidden_layer_output,hidden_layer_states = self.model(feed_previous=True)
        model.load('./model/autoencoder_model')
        return model,hidden_layer_output,hidden_layer_states
...
if __name__ == '__main__':
    ...
    if phrase == 'train':
        ...
        model,hidden_layer_output,hidden_layer_states = my_seq2seq.load()
        ...
        print("type(hidden_layer_output): ",type(hidden_layer_output))
        print("type(hidden_layer_states): ",type(hidden_layer_states))
        m2 = tflearn.DNN(hidden_layer_output, session=model.session)
        m3 = tflearn.DNN(hidden_layer_states[0], session=model.session)
        m4 = tflearn.DNN(hidden_layer_states[1], session=model.session)
        # print(m2.predict(trainXY))
        # print(m3.predict(trainXY))
        # print(m4.predict(trainXY))
        # predict /= 1000
        print("predict .........")
        # print(predict)
