...
print([word + ": " + str(word_counts[word]) for word in missing_words_list[:100]])
...
log_dir = '/tmp/tensorflow/my_seq2seq_logs/bigcorpus'
checkpoint = "./models/bigcorpus/best_model.ckpt"
...
def process_decoding_input(target_data, word_to_int):

    ending = tf.strided_slice(target_data, [0, 0], [-1, -1], [1, 1])
    go_inputs = tf.ones_like(ending)
    go_inputs = tf.slice(go_inputs, [0, 0], [-1, 1])
    decoder_input = tf.concat([go_inputs*word_to_int['<GO>'], ending], axis=1)
...
def inference_decoding_layer(embeddings, start_token, end_token, decoder_cell, initial_state,
                             output_layer, max_y_length):
    '''Create the inference logits'''
    # <GO> symbol
    start_tokens = tf.tile(tf.constant([start_token], dtype=tf.int32), [-1], name='start_tokens')
...
def decoding_layer(decoder_embed_input, embeddings, encoder_output, encoder_state,
    ...
    initial_state = decoder_cell.zero_state(y_length.get_shape()[0], tf.float32).clone(cell_state=encoder_state[0])
...
def seq2seq_model(input_data, target_data, keep_prob, X_length, y_length, max_y_length,
                  vocab_size, rnn_dim, num_layers, word_to_int):
    ...
    decoder_input = process_decoding_input(target_data, word_to_int)
    ...
    with tf.Session(graph=train_graph) as sess:
        ...
        for epoch_i in range(1, epochs+1):
            ...
            for batch_i, (y_batch, X_batch, y_lengths, X_lengths) in enumerate(
                ...
                summary_writer.add_summary(summary_str, epoch_i*(len(sorted_X_short)//batch_size) + batch_i)
                ###################################################
...
def predict():
    ...
    # checkpoint = "./best_model/models/best_model.ckpt"
