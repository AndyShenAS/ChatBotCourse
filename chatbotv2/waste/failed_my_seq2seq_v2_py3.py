...
class MySeq2Seq(object):
    ...
    def generate_trainig_data(self):
        ...
        print('length of xy_data:',len(xy_data))
    ...
    def model(self, feed_previous=False):
        ...
        decoder_inputs_tmp = tf.slice(input_data, [0, self.max_seq_len, 0], [-1, self.max_seq_len, self.word_vec_dim], name="dec_in_tmp")
        go_inputs = tf.ones_like(decoder_inputs_tmp)
        go_inputs = tf.slice(go_inputs, [0, 0, 0], [-1, 1, self.word_vec_dim])
        decoder_inputs = tf.concat([go_inputs, decoder_inputs_tmp], 1)
        #加入Go头
        if feed_previous:
            (encoder_output_tensor, states) = tflearn.lstm(encoder_inputs, self.word_vec_dim, return_state=True, scope='encoder_lstm')
            decoder_output_sequence_single = go_inputs
            decoder_output_sequence_list = []
            for i in range(self.max_seq_len+1):
                next_dec_input = decoder_output_sequence_single
                if i==0:
                    decoder_output_tensor = tflearn.lstm(next_dec_input, self.word_vec_dim, initial_state=states, return_seq=False, reuse=False, scope='decoder_lstm')
                else:
            ...
            lstm_input = tf.concat([encoder_inputs, decoder_inputs], 1)
            print('lstm_input:',lstm_input.get_shape())
            decoder_output_sequence = tflearn.lstm(lstm_input, self.word_vec_dim, return_seq=True, scope='decoder_lstm')
            decoder_output_sequence = tf.stack(decoder_output_sequence, axis=1)
            print('decoder_output_sequence:',decoder_output_sequence.get_shape())
            decoder_output_sequence = tf.slice(decoder_output_sequence, [0, self.max_seq_len, 0], [-1, self.max_seq_len+1, self.word_vec_dim], name="dec_in_tmp")


        # 编码器
        # 把encoder_inputs交给编码器，返回一个输出(预测序列的第一个值)和一个状态(传给解码器)
        # (encoder_output_tensor, states) = tflearn.lstm(encoder_inputs, self.word_vec_dim, return_state=True, scope='encoder_lstm')
        # print('encoder_output_tensor:',encoder_output_tensor.get_shape())
        # encoder_output_sequence = tf.stack([encoder_output_tensor], axis=1)
        # print('encoder_output_sequence:',encoder_output_sequence.get_shape())
        ...
        #     print('first_dec_input:',first_dec_input.get_shape())
        # decoder_output_tensor = tflearn.lstm(first_dec_input, self.word_vec_dim, initial_state=states, return_seq=False, reuse=False, scope='decoder_lstm')
        # print('decoder_output_tensor:',decoder_output_tensor.get_shape())
        # decoder_output_sequence_single = tf.stack([decoder_output_tensor], axis=1)
        # print('decoder_output_sequence_single:',decoder_output_sequence_single.get_shape())
        ...
        #         print('next_dec_input:',next_dec_input.get_shape())
        #     decoder_output_tensor = tflearn.lstm(next_dec_input, self.word_vec_dim, return_seq=False, reuse=True, scope='decoder_lstm')
        #     print('decoder_output_tensor:',decoder_output_tensor.get_shape())
        #     decoder_output_sequence_single = tf.stack([decoder_output_tensor], axis=1)
        #     print('decoder_output_sequence_single:',decoder_output_sequence_single.get_shape())
        ...
        net = tflearn.regression(real_output_sequence, optimizer='sgd', learning_rate=1, loss='mean_square')
    ...
    def train(self):
        ...
        model.fit(trainXY, trainY, n_epoch=3, snapshot_epoch=False, batch_size=1)
